...
CREATE TABLE Season_Summary (
   ...
   viewers_mean FLOAT,
   rank INT,
   FOREIGN KEY (winner_id) REFERENCES Castaway_Details(castaway_id)
);

-- 2. Composite PK tables that are referenced later
CREATE TABLE Advantage_Details (
   ...
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season)
);
...
CREATE TABLE Castaways (
   ...
   ack_quote TEXT,
   ack_score INT,
   UNIQUE (castaway_id, version_season),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id)
);

CREATE TABLE Advantage_Movement (
   advantage_movement_id SERIAL PRIMARY KEY,
   version TEXT,
   version_season VARCHAR(255),
   season_name TEXT,
   season INT,
   ...
   sog_id INT,
   UNIQUE (sequence_id, version_season, advantage_id),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id),
   FOREIGN KEY (version_season, advantage_id) REFERENCES Advantage_Details(version_season, advantage_id)
);
...
CREATE TABLE Boot_Mapping (
   ...
   tribe_status TEXT,
   game_status TEXT,
   UNIQUE (sog_id, castaway_id, version_season),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id)
);
...
CREATE TABLE Tribe_Mapping (
   ...
   tribe TEXT,
   tribe_status TEXT,
   UNIQUE (castaway_id, version_season, episode, day, tribe),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id)
);
...
CREATE TABLE Confessionals (
   ...
   exp_time FLOAT,
   UNIQUE (castaway_id, version_season, episode),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id)
);

CREATE TABLE Challenge_Results (
   challenge_results_id SERIAL PRIMARY KEY,
   version TEXT,
   version_season VARCHAR(255),
   season_name TEXT,
   season INT,
   ...
   order_of_finish INT,
   sog_id INT,
   UNIQUE (castaway_id, challenge_id, sog_id, version_season),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id)
   --FOREIGN KEY (version_season, challenge_id) REFERENCES Challenge_Description(version_season, challenge_id)
);
...
CREATE TABLE Vote_History (
   ...
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id),
   FOREIGN KEY (voted_out_id) REFERENCES Castaway_Details(castaway_id),
   FOREIGN KEY (vote_id) REFERENCES Castaway_Details(castaway_id)
   --FOREIGN KEY (version_season, challenge_id) REFERENCES Challenge_Description(version_season, challenge_id)
);
...
CREATE TABLE Jury_Votes (
   ...
   UNIQUE (castaway_id, finalist_id, version_season),
   FOREIGN KEY (version_season) REFERENCES Season_Summary(version_season),
   FOREIGN KEY (castaway_id) REFERENCES Castaway_Details(castaway_id),
   FOREIGN KEY (finalist_id) REFERENCES Castaway_Details(castaway_id)
);
